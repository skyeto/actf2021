...
void play() {
	string action;
	character player;
	printf("init to %lx %lx\n\n\n\n", player.health, player.skill);
	cout << "Enter your name: " << flush;
	getline(cin, player.name); // play()+378
	...
	while (true) {
		...
		if (action == "1") {
			...
			printf("%s %p %lx\n\n\n\n\n", player.skill,player.skill,player.skill);
...
// agreeing stored: 0x7ffffffedd50 -> 0x7ffffffedd60
// sign stored at:  0x7ffffffedd70 -> 0x801b1d0
// name          :  0x7ffffffedd70 -> 0x801b1d0
